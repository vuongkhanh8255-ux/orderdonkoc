...
export async function handleLiveSuggest(body) {
  ...
  const sys = `Bạn là biên kịch clip livestream bán hàng Shopee Việt Nam. Quy trình: video do Seedance quay (dùng ẢNH SẢN PHẨM làm tham chiếu, người trong clip do AI tự tạo từ chữ nên KHÔNG bị chặn), tiếng là voice-over lồng riêng qua CapCut. Từ yêu cầu thô của người bán, viết JSON đúng 6 khoá này:
{"mo_ta":"...","noi_dung":"...","loi_thoai":"...","goc_quay":"...","hanh_dong":"...","img_prompt":"..."}
- "mo_ta": mô tả bối cảnh + sản phẩm, 1-2 câu tiếng Việt (VD: bộ chăm sóc da thiên nhiên bày trên bàn gỗ, studio livestream sáng).
- "noi_dung": điểm bán / ưu đãi chính, tiếng Việt ngắn gọn, giữ ĐÚNG số liệu người bán đưa, không bịa khuyến mãi.
- "loi_thoai": LỜI THOẠI voice-over để CapCut đọc, ~15 giây (~45-55 chữ), tiếng Việt tự nhiên như host live: xưng "em", gọi "cả nhà", chốt bằng call-to-action bấm giỏ hàng. KHÔNG emoji/hashtag/xuống dòng.
- "goc_quay": mô tả góc quay tiếng Việt (mặc định: camera CỐ ĐỊNH, trung cảnh, không di chuyển).
- "hanh_dong": hành động người trong clip, tiếng Việt (mặc định: một host nữ Việt cầm từng sản phẩm đưa lên gần camera khoe nhãn rồi hạ xuống, mỉm cười thân thiện, cử chỉ tự nhiên).
- "img_prompt": PROMPT tiếng ANH cho Seedance, GHÉP từ mo_ta + goc_quay + hanh_dong. Bắt buộc có: a young Vietnamese woman livestream host, fixed locked-off camera, holding/raising the skincare products (from reference images) up and down to show labels, keep the products EXACTLY like the reference images, bright studio, photorealistic, smooth natural motion, no camera movement. 50-80 chữ.`;
  const usr = `Chủ đề/câu: ${label || '(không rõ)'}\nYêu cầu thô của người bán: ${idea}`;
  try {
    ...
    const S = (k) => String(out[k] || '').trim();
    // loi_thoai = script (giữ tên cũ cho Xưởng Clip); form = 4 ô mô tả
    const script = S('loi_thoai') || S('script'), img_prompt = S('img_prompt');
    if (!script || !img_prompt) return { ok: false, error: 'AI trả thiếu lời thoại/prompt — bấm thử lại.' };
    return { ok: true, script, img_prompt, form: { mo_ta: S('mo_ta'), noi_dung: S('noi_dung'), goc_quay: S('goc_quay'), hanh_dong: S('hanh_dong') } };
...
export async function handleLiveGenImage(body) {
  ...
  const shop_key = (body && body.shop_key) || 'chung';   // gian hang so huu clip (Khanh 4/8)
  ...
  try {
    ...
    let warn;
    if (intent_id) {
      const { error: e } = await store.from('livestream_clip_prod').upsert({ shop_key, intent_id, image_url, updated_at: new Date().toISOString() }, { onConflict: 'shop_key,intent_id' });
...
export async function handleLiveMakeVideo(body) {
  ...
  const shop_key = (body && body.shop_key) || 'chung';
  ...
  try {
    ...
    const uploadTalkingPhoto = async () => {
      ...
      if (intent_id) {
        const { error: e } = await store.from('livestream_clip_prod').upsert({ shop_key, intent_id, talking_photo_id: id, tp_image_url: image_url, updated_at: new Date().toISOString() }, { onConflict: 'shop_key,intent_id' });
    ...
    if (intent_id) {
      const { data: row } = await store.from('livestream_clip_prod').select('talking_photo_id, tp_image_url').eq('shop_key', shop_key).eq('intent_id', intent_id).maybeSingle();
    ...
    if (intent_id) {
      ...
      const { error: e } = await store.from('livestream_clip_prod').upsert({ shop_key, intent_id, video_id, voice_id: vid, video_url: '', status: 'lam', updated_at: new Date().toISOString() }, { onConflict: 'shop_key,intent_id' });
...
export async function handleLiveCheckVideo(body) {
  ...
  try {
    ...
    if (intent_id) {
      const { data: row } = await store.from('livestream_clip_prod').select('video_url, video_id').eq('shop_key', shop_key).eq('intent_id', intent_id).maybeSingle();
    ...
    if (d.status === 'completed' && d.video_url) {
      ...
      if (intent_id) await store.from('livestream_clip_prod').upsert({ shop_key, intent_id, video_url: finalUrl, status: 'xong', updated_at: new Date().toISOString() }, { onConflict: 'shop_key,intent_id' });
