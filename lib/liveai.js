...
// ── Liệt kê talking-photo / photo-avatar của tài khoản → biết còn mấy slot, lấy id để xoá ──
// Dò NHIỀU endpoint vì mỗi loại (talking_photo v1 vs photo_avatar_group v2) nằm chỗ khác nhau.
export async function handleLiveListAvatars() {
  const key = HEYGEN();
  if (!key) return { ok: false, error: 'Chưa cấu hình HEYGEN_API_KEY trên Vercel.' };
  const H = { 'X-Api-Key': key, Accept: 'application/json' };
  const probe = async (url) => {
    try { const { ok, status, j } = await jparse(await fetch(url, { headers: H })); const d = j?.data || {};
      return { url, status, ok, keys: d && typeof d === 'object' ? Object.keys(d) : typeof d,
        lens: d && typeof d === 'object' ? Object.fromEntries(Object.entries(d).filter(([, v]) => Array.isArray(v)).map(([k, v]) => [k, v.length])) : {},
        sample: JSON.stringify(d).slice(0, 400) };
    } catch (e) { return { url, err: e.message }; }
  };
  const avatars = await probe('https://api.heygen.com/v2/avatars');
  const groups = await probe('https://api.heygen.com/v2/photo_avatar/avatar_group/list');
  const groups2 = await probe('https://api.heygen.com/v2/avatar_group.list');
  // gom mọi talking_photo tìm được
  let tp = [];
  try { const { j } = await jparse(await fetch('https://api.heygen.com/v2/avatars', { headers: H })); tp = j?.data?.talking_photos || []; } catch {}
  return { ok: true, count: tp.length, talking_photos: tp.map(t => ({ id: t.talking_photo_id || t.id, name: t.talking_photo_name || t.name || '' })), diag: { avatars, groups, groups2 } };
}
