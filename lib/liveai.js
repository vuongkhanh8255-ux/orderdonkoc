...
function rankVoices(list) {
  ...
  const fem = (arr) => arr.filter(v => v.gender === 'Female');
  const male = (arr) => arr.filter(v => v.gender !== 'Female');
...
async function pickVietnameseVoice(key) {
  ...
  const ranked = rankVoices(j?.data?.voices || j?.data || []);
  return ranked[0]?.voice_id || null;   // giọng gốc nữ đầu tiên (Chau/Hoai/Nhung…) đọc chuẩn
}
