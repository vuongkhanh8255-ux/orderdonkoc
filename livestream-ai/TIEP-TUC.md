...
| **Bước 2 — Desktop Agent** (`agent/`) | ✅ **CHẠY THẬT với OBS (3/7)** | Nhận diện 16 câu OK; đọc config từ Supabase; **nối OBS phát clip thành công (đã thấy hình)**. Còn: nguồn comment thật + kho clip FAQ xịn. |
...
2. ~~Test OBS thật~~ ✅ **XONG (3/7):** OBS 32.1.2 + 2 scene IDLE/ANSWER + media source `ANSWER_PLAYER`, WebSocket bật (tắt auth, password rỗng). Chạy `node src/index.js --mock` gõ "gia bao nhieu" → **OBS nhảy ANSWER + PHÁT CLIP thật (đã tận mắt thấy)**. Fix quan trọng: `obs.js` phải set `is_local_file:true` (media source mặc định chế độ network → bỏ qua local_file → khung đen). Setup OBS: **Local File nếu tick tay cũng được, nhưng agent đã tự set nên khỏi**.
