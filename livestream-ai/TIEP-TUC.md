...
|---|---|---|
| **Phase 0 — Đọc comment** (`comment-reader/`) | ✅ Build xong — ⏳ **CHƯA xác nhận go/no-go** | Extension cài chạy được, panel hiện. **Chưa test bắt comment trên phòng live thật.** |
| **Bước 2 — Desktop Agent** (`agent/`) | ✅ Build xong + đã test logic OK | Chạy `npm run mock -- --dry` cho ra kết quả đúng |
| Bước 3 — Tab dashboard trong koc-tool | ⏳ Chưa làm | |
