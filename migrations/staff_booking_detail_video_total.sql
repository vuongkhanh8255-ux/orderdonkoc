-- Báo cáo nhân sự → bảng Top KOC: thêm Video TỔNG (all-time) + Video KỲ (union đơn∪bảng video,
-- khớp koc_order_stats bên Hiệu suất KOC). Filter KOC đổi videos-trong-kỳ → videos_total>0
-- (KOC gắn có video all-time vẫn hiện dù 0 hoạt động kỳ).
create or replace function public.staff_booking_detail(p_nhansu_id uuid, p_from date, p_to date)
returns jsonb language sql stable security definer
set search_path to 'public' set statement_timeout to '30s' set work_mem to '256MB'
as $function$
  ...
  k_vid as (select sa.uname, coalesce(sum(v.views),0) as views
    from sa join tiktok_shop_videos v on v.shop_id=sa.shop_id and lower(regexp_replace(coalesce(v.username,''),'^@',''))=sa.uname, rng
