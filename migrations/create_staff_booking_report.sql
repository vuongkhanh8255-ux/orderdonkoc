-- Báo cáo Booking theo nhân sự (tab "Báo cáo nhân sự").
-- Gộp Module 1 (đơn gửi/donguis) + Chi phí mẫu (cost AMIS V2 ×1.08×SL + 5k + ship) + Hiệu suất KOC affiliate
--   (qua gắn KOC koc_brand_assignments) + CAST đã dùng (koc_payments).
-- Lọc theo KHOẢNG NGÀY [p_from, p_to] (không còn tháng/năm). Trả thêm cast_used, chi_phi_mau.
-- SECURITY DEFINER vì gọi từ frontend (anon) nhưng đọc tiktok_affiliate_orders (bảng server-only).
-- Đã nghỉ việc → loại khỏi báo cáo theo id (Anh Kiệt, Trúc Linh) — xem WHERE cuối.
-- Đã apply lên DB 2026-06-17.
...
as $function$
  ...
  where (o.nhansu_id is not null or k.nhansu_id is not null or cu.nhansu_id is not null)
    -- Đã nghỉ việc → loại: Anh Kiệt (04b2e08d), Trúc Linh (d42754b4).
    -- Có người nghỉ thêm thì thêm id vào đây.
