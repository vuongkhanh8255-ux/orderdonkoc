...
-- Đồng thời: so brand bỏ qua hoa/thường + khoảng trắng (staff_booking_*) để brand gõ lệch không rớt KOC.
-- (cast_brand_shop được seed riêng trong cast_brand_shop.sql)
...
-- ── 2) Báo cáo nhân sự (drawer chi tiết) ─────────────────────────────────────
...
AS $function$
  ...
  sa as (select distinct lower(regexp_replace(a.koc_id,'^@','')) as uname, bm.shop_id
    from koc_brand_assignments a join nhansu n on lower(trim(n.ten_nhansu))=lower(trim(a.staff_name))
    left join brand_map bm on upper(trim(bm.brand_name))=upper(trim(a.brand_name))
...
-- ── 3) Cảnh báo KOC chưa air video (brand_shop dùng key chuẩn hoá → REALSTEEL) ──
...
-- ── 4) Trạng thái clip theo KOC×brand ────────────────────────────────────────
...
-- ── 6) Video cần lấp view (nhánh air_links dùng ten_brand 'REAL STEEL') ───────
