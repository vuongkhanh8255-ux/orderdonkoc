-- Báo cáo Booking theo nhân sự — bản DATE-RANGE (thay bản theo Tháng/Năm).
-- Lọc theo khoảng ngày [p_from, p_to] (như Dashboard Ecom). Thêm cột chi_phi_mau.
-- chi_phi_mau = GIỐNG Module 1 Order: cost (cột "...AMIS V2" mới nhất trong costing_data) ×1.08×SL
--               + 5000/đơn + ship (Hỏa tốc 50k / thường 20k). Tính per-order rồi sum theo nhân sự.
-- Hiệu năng: work_mem 256MB (tránh spill disk khi sort ~100k đơn affiliate),
--            cần index idx_shop_videos_shop_postdate (xem optimize_staff_booking_perf.sql).
-- SECURITY DEFINER (đọc tiktok_affiliate_orders server-only). Đã apply lên DB 2026-06-17.
drop function if exists public.staff_booking_report(integer, integer);

create function public.staff_booking_report(p_from date, p_to date)
returns table(nhansu_id uuid, ten_nhansu text, so_don bigint, so_mau numeric, so_ngay bigint, tan_suat numeric,
  top_product text, brand_dist jsonb, koc_count bigint, koc_list jsonb,
  aff_gmv numeric, aff_videos bigint, aff_views numeric, cast_used numeric, chi_phi_mau numeric)
language sql stable security definer set search_path to 'public'
set statement_timeout to '40s' set work_mem to '256MB'
as $function$
  ...
  vid as (select sa.nhansu_id, count(distinct v.id) as so_video, coalesce(sum(v.views),0) as views
  ...
  select n.id, n.ten_nhansu,
    ...
    coalesce(af.gmv,0), coalesce(vd.so_video,0), coalesce(vd.views,0), coalesce(cu.cast_used,0), coalesce(mc.chi_phi_mau,0)
  from nhansu n
  ...
  left join vid vd on vd.nhansu_id=n.id left join cst cu on cu.nhansu_id=n.id
