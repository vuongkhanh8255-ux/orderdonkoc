-- Chi tiết 1 nhân sự cho drawer "Báo cáo nhân sự (Booking)" — bản DATE-RANGE.
-- Trả về jsonb { daily:[{d,don,mau,gmv,video,views}], kocs:[{uname,gmv,views,videos,cast}] }
--   daily: chuỗi theo ngày trong khoảng [p_from, p_to] (ngày không có data = 0).
--   kocs : per-KOC GMV/view/video/cast → scatter GMV vs CAST + bảng Top KOC.
-- GMV/video theo shop của brand được gắn. post_date = ngày ĐĂNG video. order_date = ngày đơn affiliate.
-- cast per-KOC: map koc_payments.channel_link (@username) -> creator (cast là reserved keyword → cột cast_amt).
-- SECURITY DEFINER. work_mem 256MB. Đã apply lên DB 2026-06-17.
drop function if exists public.staff_booking_detail(uuid, integer, integer);

create function public.staff_booking_detail(p_nhansu_id uuid, p_from date, p_to date)
returns jsonb language sql stable security definer set search_path to 'public'
set statement_timeout to '30s' set work_mem to '256MB'
as $function$
  ...
  k_vid as (select sa.uname, count(distinct v.id) as videos, coalesce(sum(v.views),0) as views
    from sa join tiktok_shop_videos v on v.shop_id=sa.shop_id and lower(regexp_replace(coalesce(v.username,''),'^@',''))=sa.uname, rng
    where v.post_date>=rng.d0 and v.post_date<=rng.d1 group by sa.uname),
