...
as $function$
  ...
  vid_date as (
    select id::text as vid, max(post_date) as post_date
    from tiktok_shop_videos where post_date is not null group by id::text
  ),
  air_date as (
    select id_video::text as vid, max(ngay_air) as ngay_air
    from air_links where ngay_air is not null and ngay_air <= date '2026-12-31' group by id_video::text
  ),
  res as (
    select p.staff, p.cast_net, p.total, coalesce(vd.post_date, ad.ngay_air) as air_dt
    from pay p
    left join vid_date vd on vd.vid = p.vid
    left join air_date ad on ad.vid = p.vid
  )
  select staff,
         to_char(date_trunc('month', air_dt), 'YYYY-MM') as air_month,
         sum(coalesce(cast_net,0))::numeric as cast_net,
         sum(coalesce(total,0))::numeric    as total,
...
-- 2) Đơn KHÔNG suy ra được ngày air (để qua Module 5 Quản lý link air đối chiếu). Lọc theo pay_date.
