...
function App() {
  ...
  // =================================================================
  // HÀM TẢI DỮ LIỆU ĐÃ SỬA VỚI PHÂN TRANG
  // =================================================================
  ...
  const loadInitialData = async () => { // Tải data cho tab Order
    ...
    .range(startIndex, endIndex); // <--- DÙNG RANGE ĐỂ CHỈ TẢI ĐƠN HÀNG CỦA TRANG HIỆN TẠI
    ...
    if(error) { alert("Lỗi tải dữ liệu Order: " + error.message) } 
      ...
      const dataWithStt = data.map((item, index) => ({ 
          ...item, 
          originalStt: count - (startIndex + index) 
      }));
  ...
  // Tự động tải lại đơn hàng khi đổi trang
  useEffect(() => {
      // Ngăn không cho chạy khi component mới được mount lần đầu
      if (currentPage >= 1) { 
          loadInitialData();
      }
  }, [currentPage]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Quay về trang 1 và tải lại data
      setCurrentPage(1); 
      // Không cần loadInitialData ở đây vì useEffect [currentPage] sẽ tự gọi
