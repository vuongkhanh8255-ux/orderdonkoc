...
function App() {
  ...
  const [sanPhams, setSanPhams] = useState([]); // Dùng cho form Order
  const [currentView, setCurrentView] = useState('orders'); // 'orders' hoặc 'booking'
  ...
  const [bookingSanPhams, setBookingSanPhams] = useState([]); // SP cho form booking
  ...
  const loadInitialData = async () => { // Tải data cho tab Order
    const { data, error } = await supabase.from('donguis').select(`
      ...
      chitiettonguis ( id, so_luong, sanphams ( id, ten_sanpham, barcode, gia_tien, brands ( id, ten_brand ) ) )
  ...
  // HÀM MỚI: Tải data cho tab Booking
  const loadBookings = async () => {
      setIsLoadingBookings(true);
      // SỬA LỖI: Dùng alias nguoi_deal_id:nhansu để join đúng
      // Lưu ý: Tên cột `nguoi_deal_id` trong code phải khớp 100% với tên cột bạn tạo trong Supabase (ví dụ: `Nhan_su`?)
      const { data, error } = await supabase
          .from('bookings') // Đảm bảo tên bảng là 'bookings'
          .select(`
              *,
              brands ( ten_brand ),
          ...
          `)
          .order('Ngay_booking', { ascending: false }); // Khớp tên cột 'Ngay_booking'
  ...
  // HÀM MỚI: Xử lý submit form booking
  const handleBookingSubmit = async (event) => {
      event.preventDefault();
      setIsBookingLoading(true);

      // SỬA LỖI: Khớp chính xác tên cột trong Supabase
      const bookingData = {
          Koc_name: bookingKocName,
          Cast: bookingCastCmt,
          Ngay_air: bookingNgayAir || null,
          Link_video: bookingLinkVideo,
          Brand_id: bookingBrandId,
          san_pham_id: bookingSanPhamId || null,
          key_content: bookingKeyContent,
          doi_tuong: bookingDoiTuong,
          chat_luong: bookingChatLuong,
          dat_kpi: bookingDatKpi,
          Nhan_su: bookingNguoiDealId, // Khớp với tên cột "Nhan_su"
          Ngay_booking: bookingNgayBooking || null,
          Cms: bookingCms || null,
          Id_video: bookingIdVideo,
          Thong_tin_koc: bookingThongTinKoc, // Khớp với tên cột "Thong_tin_ko..."
      };
      ...
      try {
          ...
          await loadBookings(); // Tải lại danh sách booking mới nhất
      ...
      <div style={{ marginBottom: '2rem', textAlign: 'center' }}>
          ...
          <button 
              ...
              style={{ 
                  ...
                  fontSize: '16px', 
                  cursor: 'pointer',
                  backgroundColor: currentView === 'booking' ? '#007bff' : '#f8f9fa',
                  color: currentView === 'booking' ? 'white' : 'black',
                  border: '1px solid #dee2e6',
                  borderRadius: '5px'
              }}
          >
                    ...
                    <div style={{ marginTop: '1rem', maxHeight: '300px', overflowY: 'auto' }}>
                        ...
                        {rawSummaryData.length > 0 && 
                            <div style={{ marginTop: '1rem', textAlign: 'right' }}>
                                <button onClick={() => handleExport({ data: rawSummaryData, headers: summaryExportHeaders, filename: `tong-hop-san-pham-${summaryDate}.xlsx`})} style={{ padding: '8px 16px', backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Xuất File Tổng Hợp (Ship)</button>
                            </div>
                        }
                    </div>
                </div>
          ...
          <div style={{ display: 'flex', gap: '2rem' }}>
              ...
              <div style={{ flex: 1, padding: '2rem', border: '1px solid #ddd', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                  ...
                  <form onSubmit={handleBookingSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                      ...
                      <div><label>Người Deal</label><select value={bookingNguoiDealId} onChange={e => setBookingNguoiDealId(e.target.value)} style={{ width: '100%', padding: '8px', boxSizing: 'border-box' }} required>
                      ...
                      <div><label>Đối tượng</label><input type="text" value={bookingDoiTuong} onChange={e => setBookingDoiTuong(e.target.value)} style={{ width: '100%', padding: '8px', boxSizing: 'border-box' }} /></div>
                      ...
                      <button type="submit" disabled={isBookingLoading} style={{ width: '100%', padding: '10px', backgroundColor: isBookingLoading ? '#ccc' : '#28a745', color: 'white', border: 'none', borderRadius: '4px', fontSize: '16px', cursor: 'pointer', marginTop: '1rem' }}>
                          {isBookingLoading ? 'Đang lưu...' : 'Lưu Booking'}
                      </button>
                  </form>
              </div>

              {/* --- CỘT BẢNG HIỂN THỊ BOOKING --- */}
              <div style={{ flex: 2, padding: '2rem', border: '1px solid #ddd', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                  <h2 style={{ textAlign: 'center', color: '#333', marginBottom: '1.5rem' }}>Danh Sách Booking</h2>
                  {isLoadingBookings ? <p>Đang tải dữ liệu booking...</p> : (
                                  ...
                                  <tbody>
                                      {bookings.map((booking) => (
                                          <tr key={booking.id}>
                                              ...
                                              <td style={{ padding: '12px', border: '1px solid #ddd' }}>{booking.sanphams?.ten_sanpham || 'N/A'}</td>
                                              {/* Sửa: Khớp tên cột Ngay_air */}
                                              <td style={{ padding: '12px', border: '1px solid #ddd' }}>{booking.Ngay_air ? new Date(booking.Ngay_air).toLocaleDateString('vi-VN') : ''}</td>
                                              {/* Sửa: Khớp tên cột Link_video */}
                                              ...
                                              {/* Sửa: Khớp tên cột dat_kpi */}
                                              <td style={{ padding: '12px', border: '1px solid #ddd', textAlign: 'center' }}>{booking.dat_kpi ? '✅' : '❌'}</td>
                                          </tr>
                                      ))}
                                  </tbody>
                              </table>
