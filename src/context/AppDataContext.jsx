  ...
  const handleGetSummary = async () => {
    ...
    try {
        ...
        const { data, error } = await supabase
            ...
            .select(`
                so_luong,
                donguis!inner ( loai_ship, ngay_gui ),
                sanphams ( ten_sanpham, barcode, brands ( ten_brand ) )
            `)
            .gte('donguis.ngay_gui', startDate)
            .lte('donguis.ngay_gui', endDate);
        ...
        setProductSummary({
            'Ship thường': finalData.filter(item => item.loai_ship === 'Ship thường'),
            'Hỏa tốc': finalData.filter(item => item.loai_ship === 'Hỏa tốc')
        });
