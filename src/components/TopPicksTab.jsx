...
export default function TopPicksTab() {
  ...
  const loadBoostedList = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        setBoostedItems(res.data?.boosted_item_list || res.data?.items || res.data || []);
      }
    } catch { /* silent */ }
  }, []);
  ...
  useEffect(() => {
    ...
    loadBoostedList();
    loadCollections();
  }, [loadProducts, loadBoostedList, loadCollections]);
...
// BOOST SECTION
// ══════════════════════════════════════════════════════════════════════════════
function BoostSection({ products, boostedItems, boostSelected, onToggleSelect, onBoost, onRefreshBoosted, onLoadMore, hasMoreProducts, loadingProducts }) {
