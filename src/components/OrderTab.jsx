...
const OrderTab = () => {
  ...
  // Logic phân trang (chuyển từ App.js qua)
  ...
  // Headers cho bảng (chuyển từ App.js qua)
    ...
    <> 
      ...
      <div style={{ display: 'flex', gap: '2rem', marginBottom: '2rem' }}>
        ...
        <div style={{ flex: 1, padding: '2rem', border: '1px solid #ddd', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div><label style={{fontWeight: 'bold', color: '#27AE60'}}>ID Kênh</label><input type="text" value={idKenh} onChange={e => setIdKenh(e.target.value)} onBlur={handleIdKenhBlur} style={{ width: '100%', padding: '8px', boxSizing: 'border-box', border: '1px solid #27AE60', borderRadius: '4px' }} required /></div>
            ...
            <div><label style={{fontWeight: 'bold', color: '#27AE60'}}>Số điện thoại</label><input type="text" value={sdt} onChange={e => setSdt(e.target.value)} style={{ width: '100%', padding: '8px', boxSizing: 'border-box', border: '1px solid #27AE60', borderRadius: '4px' }} required /></div>
            <div><label style={{fontWeight: 'bold', color: '#27AE60'}}>Địa chỉ</label><input type="text" value={diaChi} onChange={e => setDiaChi(e.target.value)} style={{ width: '100%', padding: '8px', boxSizing: 'border-box', border: '1px solid #27AE60', borderRadius: '4px' }} required /></div>
            ...
            <div><label style={{fontWeight: 'bold', color: '#27AE60'}}>Nhân sự gửi</label><select value={selectedNhanSu} onChange={e => setSelectedNhanSu(e.target.value)} style={{ width: '100%', padding: '8px', boxSizing: 'border-box', border: '1px solid #27AE60', borderRadius: '4px' }} required><option value="">-- Chọn nhân sự --</option>{nhanSus.map(nhansu => (<option key={nhansu.id} value={nhansu.id}>{nhansu.ten_nhansu}</option>))}</select></div>
            <div>
            ...
            <button type="submit" disabled={isLoading || isPastDeadlineForNewOrders} style={{ width: '100%', padding: '10px', backgroundColor: (isLoading || isPastDeadlineForNewOrders) ? '#ccc' : '#C0392B', color: 'white', border: 'none', borderRadius: '4px', fontSize: '16px', cursor: 'pointer', marginTop: '1rem', fontWeight: 'bold' }}>{isLoading ? 'Đang xử lý...' : 'Gửi Đơn'}</button>
        ...
        <div style={{ flex: 1, padding: '2rem', border: '1px solid #ddd', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}><input type="date" value={summaryDate} onChange={e => setSummaryDate(e.target.value)} style={{ padding: '7px', boxSizing: 'border-box', flex: 1, border: '1px solid #27AE60', borderRadius: '4px' }} /><button onClick={handleGetSummary} disabled={isSummarizing} style={{ padding: '8px 16px', backgroundColor: '#C0392B', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>{isSummarizing ? 'Đang xử lý...' : 'Tổng hợp'}</button></div>
            <div style={{ marginTop: '1rem', maxHeight: '300px', overflowY: 'auto' }}>
                ...
                {productSummary['Ship thường'].length > 0 && (
                    <div style={{marginBottom: '1.5rem'}}>
                        <h3 style={{color: '#27AE60', borderBottom: '1px solid #ddd', paddingBottom: '5px', fontWeight: 'bold'}}>Tổng hợp Ship Thường</h3>
                ...
                {rawSummaryData.length > 0 && 
                    <div style={{ marginTop: '1rem', textAlign: 'right' }}>
                        <button onClick={() => handleExport({ data: rawSummaryData, headers: summaryExportHeaders, filename: `tong-hop-san-pham-${summaryDate}.xlsx`})} style={{ padding: '8px 16px', backgroundColor: '#27AE60', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>Xuất File Tổng Hợp (Ship)</button>
                    </div>
                }
            </div>
        </div>
      ...
      <div style={{ padding: '2rem', border: '1px solid #ddd', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem' }}>
            ...
            <button onClick={handleGenerateReport} disabled={isReportLoading} style={{ padding: '8px 16px', backgroundColor: '#C0392B', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>
                {isReportLoading ? 'Đang xử lý...' : 'Tạo Báo Cáo'}
            </button>
        </div>
              ...
              <thead style={{ backgroundColor: '#27AE60', color: 'white' }}>
                <tr>
                  ...
                  <th style={{ padding: '12px', border: '1px solid white', textAlign: 'center', cursor: 'pointer' }} onClick={() => requestSort('chi_phi_tong')}>
                    Chi Phí Tổng {sortConfig.key === 'chi_phi_tong' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                  </th>
              ...
              <tbody>
                {sortedReportRows.map((item) => (
                   <tr key={item.nhansu_id}>
                      ...
                      <td style={{ padding: '12px', border: '1px solid #ddd', textAlign: 'center' }}>{item.sl_order}</td>
              ...
              <tfoot>
               {totalsRow && (
                  <tr style={{backgroundColor: '#f8d7da', fontWeight: 'bold', color: '#C0392B'}}>
                    ...
                    {reportData.brandHeaders.map(brand => (
                      <td key={brand} style={{ padding: '12px', border: '1px solid #ddd', textAlign: 'center' }}>
                        {totalsRow.brand_counts[brand] || 0}
                      </td>
                    ))}
                  </tr>
                 )}
      ...
      <div style={{ width: '100%', overflowX: 'auto', marginTop: '2rem' }}>
        <h2 style={{ textAlign: 'center', color: '#C0392B', borderBottom: '2px solid #C0392B', paddingBottom: '10px' }}>Danh Sách Đơn Hàng Đã Gửi</h2>
         <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem', alignItems: 'end', marginBottom: '1rem', padding: '1rem', backgroundColor: '#f9f9f9', borderRadius: '8px', border: '1px solid #C0392B' }}>
            <input type="text" placeholder="ID kênh..." value={filterIdKenh} onChange={e => setFilterIdKenh(e.target.value)} style={{ padding: '8px', boxSizing: 'border-box', border: '1px solid #27AE60', borderRadius: '4px' }} />
            ...
            <select value={filterBrand} onChange={e => setFilterBrand(e.target.value)} style={{ padding: '8px', boxSizing: 'border-box', border: '1px solid #27AE60', borderRadius: '4px' }}><option value="">Tất cả Brand</option>{brands.map(b => <option key={b.id} value={b.id}>{b.ten_brand}</option>)}</select>
            <select value={filterSanPham} onChange={e => setFilterSanPham(e.target.value)} style={{ padding: '8px', boxSizing: 'border-box', border: '1px solid #27AE60', borderRadius: '4px' }} disabled={!filterBrand}><option value="">Tất cả Sản phẩm</option>{filterSanPhams.map(sp => <option key={sp.id} value={sp.id}>{sp.ten_sanpham}</option>)}</select>
            ...
            <div style={{display: 'flex', gap: '0.5rem'}}>
              ...
              <button onClick={handleBulkUpdateStatus} disabled={selectedOrders.size === 0} style={{ flex: 1, padding: '8px 16px', backgroundColor: selectedOrders.size > 0 ? '#C0392B' : '#ccc', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>
        ...
        <div style={{ textAlign: 'center', marginTop: '1.5rem', marginBottom: '1.5rem' }}>
            ...
            <button onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1 || isLoading} style={{ padding: '8px 12px', margin: '0 5px', border: '1px solid #C0392B', cursor: 'pointer', backgroundColor: '#f8f9fa', color: '#C0392B', borderRadius: '4px' }}>
                Trang Trước
            </button>
            {pageNumbers.map(number => (
                <button
                    ...
                    disabled={isLoading}
                    style={{ padding: '8px 12px', margin: '0 5px', backgroundColor: currentPage === number ? '#C0392B' : '#f8f9fa', color: currentPage === number ? 'white' : '#C0392B', border: '1px solid #C0392B', cursor: 'pointer', borderRadius: '4px' }}
                >
                    {number}
                </button>
            ))}
            <button onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages || isLoading} style={{ padding: '8px 12px', margin: '0 5px', border: '1px solid #C0392B', cursor: 'pointer', backgroundColor: '#f8f9fa', color: '#C0392B', borderRadius: '4px' }}>
                Trang Sau
            </button>
        </div>
        ...
        <div style={{ width: '100%', overflow: 'auto' }}>
            ...
            <tbody>
              {donHangs.map((donHang) => {
                const getCellStyle = (currentValue, originalValue) => (originalValue !== null && currentValue !== originalValue) ? { backgroundColor: '#C0392B', color: 'white' } : {};
                ...
                <tr key={donHang.id}>
                  ...
                  <>
                      ...
                      <td style={{ width: `${columnWidths.hanhDong}px`, padding: '12px', border: '1px solid #ddd' }}><button onClick={() => handleEdit(donHang)} style={{padding: '5px', backgroundColor: '#C0392B', color: 'white', border: 'none', borderRadius: '4px'}}>Sửa</button></td>
                  </>
                  )}
                </tr>
                );
