...
import { supabase } from '../supabaseClient'; // Gọi trực tiếp Supabase ở đây

const ContractTab = () => {
  // Lấy các hàm cũ từ Context (Giữ nguyên logic tạo hợp đồng)
  const {
    contractData,
    isOutputVisible,
    copyMessage,
  ...
  // --- PHẦN MỚI: QUẢN LÝ LINK HỢP ĐỒNG (Viết riêng tại đây) ---
  ...
  const handleAddLinkSubmit = async (e) => {
      ...
      if (error) {
          ...
          setNewLinkData({ ...newLinkData, id_kenh: '', link_hop_dong: '', cast_value: '' }); // Reset form
          loadSavedContracts(); // Tải lại bảng
      }
  };
  ...
  // Tự động tải dữ liệu khi mở Tab này
  useEffect(() => {
      loadSavedContracts();
  }, []);
  ...
  return (
    <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '1rem', fontFamily: 'Inter, sans-serif' }}>
        ...
        {/* ĐÃ XÓA CỤM TIÊU ĐỀ TO Ở ĐÂY THEO YÊU CẦU */}

        {/* ================================================= */}
        {/* PHẦN 1: CÔNG CỤ TẠO HỢP ĐỒNG (GIỮ NGUYÊN) */}
        {/* ================================================= */}
        <main style={{ display: 'grid', gridTemplateColumns: isOutputVisible ? '1fr 1fr' : '1fr', gap: '2rem', transition: 'grid-template-columns 0.3s ease-in-out', marginBottom: '3rem' }}>
            ...
            <div className="christmas-card">
                ...
                <form onSubmit={handleGenerateContract} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    <fieldset style={{ border: '1px solid #27AE60', padding: '1rem', borderRadius: '0.5rem', backgroundColor: '#f0fff0' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(1, 1fr)', gap: '1rem', marginTop: '0.5rem' }}>
                    ...
                    <fieldset style={{ border: '1px solid #C0392B', padding: '1rem', borderRadius: '0.5rem', backgroundColor: '#fff5f5' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem', marginTop: '0.5rem' }}>
                            ...
                            <div><label>Ngân hàng</label><input type="text" id="benB_nganHang" value={contractData.benB_nganHang} onChange={handleContractFormChange} required /></div>
            ...
            {/* KẾT QUẢ HỢP ĐỒNG */}
            <div id="outputContainer" className="christmas-card" style={{ display: isOutputVisible ? 'block' : 'none', border: '2px solid #27AE60' }}>
        ...
        {/* ========================================================= */}
        {/* PHẦN 2: KHO LƯU TRỮ LINK HỢP ĐỒNG (MỚI THÊM) */}
        {/* ========================================================= */}
        <div className="christmas-card">
                            ...
                            savedContracts.map((item, index) => (
                                <tr key={item.id}>
                                    ...
                                    <td style={{ textAlign: 'center' }}>
                                        <button 
                                            onClick={() => deleteContractLink(item.id)}
                                            style={{ backgroundColor: '#D42426', padding: '5px 10px', fontSize: '12px' }}
                                        >
                                            Xóa
                                        </button>
                                    </td>
